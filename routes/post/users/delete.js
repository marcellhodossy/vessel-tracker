...
router.get('/users/groups/:id/routes/:routes/delete', async (req,res) => {
    ...
    await pool.query("DELETE FROM routes WHERE id = $1 AND group_id = $2", [req.params.routes, req.params.id])
    return res.redirect(`/users/groups/${req.params.id}`);
});
